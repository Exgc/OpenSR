...
class HubertEncoderWrapper(FairseqEncoder):
    def __init__(self, w2v_model, prompting, strategy='base'):
        super().__init__(None)
    ...
    def forward(self, source, padding_mask, **kwargs):
        ...
        if self.prompting:
            x, padding_mask = self.w2v_model.extract_prompt(**w2v_args)
        else:
            x, padding_mask = self.w2v_model.extract_finetune(**w2v_args)
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
...
class AVHubertSeq2Seq(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, cfg, task):
        ...
        encoder = HubertEncoderWrapper(encoder_, cfg.prompting, cfg.prompt_strategy)
        if state is not None and not cfg.no_pretrained_weights:
            # set strict=False because we omit some modules
        ...
        print('%.2fM  %.2fM' % (all_params / 1e6, trainable_params / 1e6))
