...
def _main(cfg, output_file, type='sent'):
    ...
    for sample in progress:
        ...
        gen_timer.start()
        hypos, logits = task.inference_step(
            generator,
            models,
            sample,
        ...
        for i in range(len(sample["id"])):
            ...
            if type == 'sent':
                ...
                best_hypo = torch.argmax(logits[i].mean(0)).int().cpu()
            hypo_str = decode_fn(best_hypo)
            result_dict['hypo'].append(hypo_str)
            with open('/mnt/lvmdisk/chengxize/res.txt', 'a+') as f:
                f.write(f"\nREF:{ref_sent}\nHYP:{hypo_str}\n")
            logger.info(f"\nREF:{ref_sent}\nHYP:{hypo_str}\n")
    ...
    if type == 'sent':
        get_sent_res(cfg=cfg, num_sentences=num_sentences, gen_timer=gen_timer, result_dict=result_dict)
    else:
        get_word_res(cfg=cfg, num_sentences=num_sentences, gen_timer=gen_timer, result_dict=result_dict)
...
if __name__ == "__main__":
    ...
    python -B infer_s2s.py \
        ...
        common_eval.path=/mnt/e5254a2d-db6d-420a-b4ea-ee215b9c32a3/chengxize/project/openLR/av_hubert/
    """
