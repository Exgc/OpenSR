...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        self.audio_root, self.names, inds, tot, self.sizes = load_audio_visual(manifest_path, max_keep_sample_size,
        ...
        self.noise_wav, self.noise_prob, self.noise_snr, self.noise_num = [ln.strip() for ln in open(
            noise_fn).readlines()] if noise_fn is not None else [], noise_prob, noise_snr, noise_num
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            #     noise_id = np.random.randint(0, 8)
            #     audio_feats = np.load(audio_fn.replace('.wav', f'_noise{noise_id}.npy'))
