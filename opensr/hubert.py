...
class Prompt_embedding(nn.Module):
    def __init__(self, encoder_embed_dim):
        ...
        self.emb_center = nn.Parameter(torch.empty(self.encoder_embed_dim))
        nn.init.normal_(self.emb_center, std=64 ** -0.5)

    def forward(self, x):
        # print(self.emb_center)
        return x + self.emb_center


class Prompt_Base(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        ...
        self.meta = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(self.encoder_embed_dim, self.center_K))
        ]))
    ...
    def forward(self, x):
        soft_weight = self.meta(x)  # B,T,C
        soft_weight = torch.softmax(soft_weight, dim=-1)
...
class PromptLearner(nn.Module):
    def __init__(self, encoder_embed_dim, strategy, training):
        ...
        if self.strategy == 'modal_prompt':
            prompt_embed_dict = {}
            for layer in range(0, 24):
                prompt_embed_dict.setdefault(str(layer), Prompt_embedding(self.encoder_embed_dim))

            self.prompt_embed = nn.ModuleDict(prompt_embed_dict)
            self.prompt_embeddings = ('modal', self.prompt_embed)
        elif 'base' in self.strategy:
            center_K = int(self.strategy.split('_')[-1])
            prompt_embed_dict = {}
            for layer in range(0, 24):
                prompt_embed_dict.setdefault(str(layer), Prompt_Base(self.encoder_embed_dim, center_K=center_K))

            self.prompt_embed = nn.ModuleDict(prompt_embed_dict)
            self.prompt_embeddings = ('base', self.prompt_embed)
        else:
...
class AVHubertModel(BaseFairseqModel):
    ...
    def prompt_init(self, strategy=None):
        self.prompt_learner = PromptLearner(self.encoder_embed_dim, strategy, training=self.training)

    def extract_prompt(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None):
        src_audio, src_video = source['audio'], source['video']
        ...
        features_video, features_audio = self.prompt_learner(features_video, features_audio)
        ...
        x, _ = self.encoder(
            ...
            padding_mask=padding_mask,
            layer=None if output_layer is None else output_layer - 1,
            prompt_embedding=self.prompt_learner.prompt_embeddings
        )
