...
def trim_video_frame(csv_fn, raw_dir, output_dir, ffmpeg, rank, nshard):
    ...
    for raw_fid, fid_info in tqdm(fid_info_shard):
        ...
        for fid, start_sec, end_sec in fid_info:
            ...
            cmd = [ffmpeg, "-i", sub_dir + '/%0' + str(decimal) + 'd.png', "-y", "-crf", "20", output_path, "-loglevel",
                   "quiet"]
...
def trim_audio(csv_fn, raw_dir, output_dir, ffmpeg, rank, nshard):
    ...
    for raw_fid, fid_info in tqdm(fid_info_shard):
        ...
        cmd = ffmpeg + " -i " + os.path.join(raw_dir,
                                             raw_fid + '.mp4') + " -f wav -vn -y " + wav_path + ' -loglevel quiet'
        subprocess.call(cmd, shell=True)
...
def trim_pretrain(root_dir, ffmpeg, rank=0, nshard=1, step=1):
    ...
    if step == 1:
        ...
        output_video_dir, output_audio_dir = os.path.join(root_dir, 'short-pretrain'), os.path.join(root_dir,
                                                                                                    'audio/short-pretrain/')
...
def get_file_label(lrs2_root):
    ...
    for split in ['main']:
        subdirs = os.listdir(os.path.join(lrs2_root, split))
        for subdir in tqdm(subdirs):
            video_fns = glob.glob(os.path.join(lrs2_root, split, subdir, '*mp4'))
            video_ids = ['/'.join(x.split('/')[-3:])[:-4] for x in video_fns]
            for video_id in video_ids:
                txt_fn = os.path.join(lrs2_root, video_id + '.txt')
    ...
    for video_id, label in zip(df['id'], df['text']):
        ...
        labels_total.append(label)
    video_id_fn, label_fn = os.path.join(lrs2_root, 'file.list'), os.path.join(lrs2_root, 'label.list')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='LRS2 preprocess pretrain dir',
    ...
    if args.step <= 2:
        ...
        print(f"Extracting audio for main")
...
python lrs2_prepare.py --lrs2 /mnt/e5254a2d-db6d-420a-b4ea-ee215b9c32a3/chengxize/data/lrs2 --step 1

python lrs2_prepare.py --lrs2 /mnt/e5254a2d-db6d-420a-b4ea-ee215b9c32a3/chengxize/data/lrs2 --rank 3 --nshard 4 --step 2

python lrs2_prepare.py --lrs2 /mnt/e5254a2d-db6d-420a-b4ea-ee215b9c32a3/chengxize/data/lrs2 --rank 0 --nshard 1 --step 3

python lrs2_prepare.py --lrs2 /mnt/e5254a2d-db6d-420a-b4ea-ee215b9c32a3/chengxize/data/lrs2 --rank 3 --nshard 4 --step 4
