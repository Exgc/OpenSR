...
def make_musan_tsv(musan_root, output_dir):
    ...
    for category in ['babble', 'music', 'noise']:
        ...
        num_train, num_valid, num_test = int(0.8 * len(target_fns)), int(0.1 * len(target_fns)), int(
            0.1 * len(target_fns))
        if category in {'music', 'noise'}:
            np.random.shuffle(target_fns)
            train_fns, valid_fns, test_fns = target_fns[:num_train], target_fns[
                                                                     num_train:num_train + num_valid], target_fns[
                                                                                                       num_train + num_valid:]
...
def combine(input_tsv_dirs, output_dir):
    ...
    for input_tsv_dir in input_tsv_dirs:
        train_fn, valid_fn, test_fn = [ln.strip() for ln in open(f"{input_tsv_dir}/train.tsv").readlines()], [ln.strip()
                                                                                                              for ln in
                                                                                                              open(
                                                                                                                  f"{input_tsv_dir}/valid.tsv").readlines()], [
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Set up noise manifest',
