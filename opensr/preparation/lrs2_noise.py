...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Generating babble and speech noise from LRS2',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--lrs2', type=str, help='lrs2 root dir')
    args = parser.parse_args()
    tsv_fn = os.path.join(args.lrs2, '433h_data', 'train.tsv')

    output_wav = os.path.join(args.lrs2, 'noise', 'babble', 'noise.wav')
    output_tsvs = [os.path.join(args.lrs2, 'noise', 'babble', 'valid.tsv'),
                   os.path.join(args.lrs2, 'noise', 'babble', 'test.tsv')]
    ...
    speech_tsv_dir, speech_wav_dir = os.path.join(args.lrs2, 'noise', 'speech'), os.path.join(args.lrs2, 'noise',
                                                                                              'speech', 'wav')
